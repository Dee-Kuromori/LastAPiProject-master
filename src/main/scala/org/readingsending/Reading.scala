...
object Reading extends App {
  val spark = SparkSession.builder.appName("KafkaToJsonDemetric").master("local[*]").getOrCreate()
  ...
  df.writeStream.format("csv").option("checkpointLocation", "/tmp/jenkins/Demetric_Kafka").option("path", "/tmp/jenkins/Demetric_Kafka/FraudApib").start().awaitTermination()
